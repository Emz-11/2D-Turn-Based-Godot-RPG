...
# Same scene used for the original combat scene but for the boss encounter

# Ready relevant nodes on load/ready (references)
@onready var player_sprite: AnimatedSprite2D = $CombatPlayer
@onready var boss_sprite: AnimatedSprite2D = $BossEnemy
...
@onready var boss_hp_label: Label = $UI/BossIcon/BossEnemyHPLabel
...
# Setting up Game State variable starting with Player and Enemy HP stats
var boss_enemy_hp: int = 500
var boss_enemy_max_hp: int = 500
# Remembers starting positions for later use
var player_original_pos: Vector2
var boss_enemy_original_pos: Vector2
...
func _ready() -> void:
	...
	boss_enemy_original_pos = boss_sprite.position
	...
	# Connects the button presses to their respective functions
...
func update_hp_ui() -> void:
	...
	boss_hp_label.text = "Enemy HP: %d / %d" % [boss_enemy_hp, boss_enemy_max_hp]
	boss_hp_label.add_theme_color_override("font_color", Color.RED) if boss_enemy_hp < 30 else Color.WHITE
...
func start_enemy_turn() -> void:
	...
	# Small pause between player and enemy actions
	await get_tree().create_timer(1).timeout
	if boss_enemy_hp > 0:
		enemy_attack()

# Checks health after every attack, ends the battle when an entity dies
func check_win_condition() -> bool:
	if boss_enemy_hp <= 0:
		...
		world_state.player_experience += 1500
...
func player_attack() -> void:
	...
	# Player character will move 470 pixels in 0.35 seconds towards the enemy then attack
	...
	tween.tween_callback(func():
		...
		boss_enemy_hp = max(0, boss_enemy_hp - damage)
		update_hp_ui()
		
		damage_number.spawn_label(damage, crit_chance, false, boss_sprite.global_position)
		
		# Enemy reaction to being hit
		var hit_tween = create_tween()
		hit_tween.tween_property(boss_sprite, "scale", Vector2(1.45, 0.65), 0.10)
		hit_tween.tween_property(boss_sprite, "scale", Vector2(1.0, 1.0), 0.12)
	)
...
func player_heal() -> void:
	...
	var tween = create_tween()
	# Player character will move 1 pixel in 0.55 seconds towards the enemy then attack
...
func enemy_attack() -> void:
	...
	var tween = create_tween()
	# Enemy character will move 470 pixels in 0.35 seconds towards the player then attack
	tween.tween_property(boss_sprite, "position", Vector2(boss_enemy_original_pos.x - 470, boss_enemy_original_pos.y), 0.55)\
		 .set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	
	# Damage logic and log update, happens after moving first
	tween.tween_callback(func():
		var damage = randi_range(26, 30) - 0.40*(world_state.player_armor)
	...
	tween.tween_property(boss_sprite, "position", boss_enemy_original_pos, 0.45)\
		 .set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
